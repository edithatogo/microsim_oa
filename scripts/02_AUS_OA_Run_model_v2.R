...
run_simulation <- function(simulation_config, model_coefficients,
                           comorbidity_params, intervention_params,
                           custom_coeffs = NULL, seed = NULL) {
  ...
  if (run_modes$calibration_mode) {
    ...
    eq_cust <- list(
      BMI = data.frame(covariate_set = c("c1", "c2", "c3", "c4", "c5"),
                       proportion_reduction = c(1, 1, 1, 1, 1)),
      OA = data.frame(covariate_set = "cons", proportion_reduction = 1),
      TKR = data.frame(
        covariate_set = c("c9_cons", "c9_age", "c9_age2", "c9_drugoa", "c9_ccount",
...
if (sys.nframe() == 0) {
  ...
  am_all <- run_simulation(simulation_config, model_parameters,
                           comorbidity_parameters, intervention_parameters,
                           seed = 123)
}
