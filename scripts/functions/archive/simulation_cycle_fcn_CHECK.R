...
simulation_cycle_fcn <- function(am_curr, cycle.coefficents, am_new,
  ...
  TKA_update_data <- TKA_update_fcn(am_curr,am_new, cycle.coefficents, TKR_cust, summary_TKR_observed_diff)
  ...
  # find each individuals mort risk given current age
  
  # eventually this should be moved out of the loop, but should still be faster than the current
  # for loop over am object
  
  current.mort.data <- as.data.frame(cbind(1:101,lt$male_sep1_bmi0,lt$female_sep1_bmi0))
  names(current.mort.data) <- c("age", "male_sep1_bmi0", "female_sep1_bmi0")
  
  am_curr <- merge(am_curr,current.mort.data,by="age")
  
  am_curr$qx_test <- am_curr$male_sep1_bmi0 * am_curr$male +
    am_curr$female_sep1_bmi0 * (1-am_curr$male)
