# Simple data generation script using base R
set.seed(12345)
...
generate_longitudinal_data <- function(n_patients = 5000, n_visits = 10) {
  ...
  for (patient in patients) {
    ...
    for (visit in 1:n_visits) {
      ...
      visit_date <- as.Date("2020-01-01") + (visit - 1) * 90 + rnorm(1, 0, 14)
      ...
      longitudinal_data <- rbind(longitudinal_data, visit_data)
    }
  }

  # Add missing data
  missing_prob <- 0.05
  n_rows <- nrow(longitudinal_data)
  longitudinal_data$pain_score <- ifelse(runif(n_rows) < missing_prob,
                                       NA, longitudinal_data$pain_score)
  longitudinal_data$functional_score <- ifelse(runif(n_rows) < missing_prob,
                                             NA, longitudinal_data$functional_score)
  longitudinal_data$qol_score <- ifelse(runif(n_rows) < missing_prob,
...
cat("Generated", nrow(data), "observations for", length(unique(data$patient_id)), "patients\n")
