  ...
  for (intervention_name in names(intervention_params$interventions)) {
    ...
    if (year >= intervention$start_year && year <= intervention$end_year) {
      ...
      if (intervention$type == "bmi_modification") {
        attribute_matrix <- apply_bmi_modification(attribute_matrix, intervention)
      } else if (intervention$type == "qaly_and_cost_modification") {
        attribute_matrix <- apply_qaly_cost_modification(attribute_matrix, intervention)
      }
...
apply_bmi_modification <- function(am, intervention) {
  ...
  affected_indices <- target_indices & (runif(nrow(am)) < intervention$parameters$uptake_rate)
  
  # Apply the BMI change
  am[affected_indices, "bmi"] <- am[affected_indices, "bmi"] + intervention$parameters$bmi_change
...
apply_qaly_cost_modification <- function(am, intervention) {
  ...
  # Apply the QALY gain
  am[affected_indices, "d_sf6d"] <- am[affected_indices, "d_sf6d"] + intervention$parameters$qaly_gain
  ...
  am[affected_indices, "intervention_cost"] <- am[affected_indices, "intervention_cost"] + intervention$parameters$annual_cost
