...
#' based on their current health state, including KL grade, BMI, and comorbidities.
#'
...
calculate_qaly <- function(attribute_matrix, utility_params) {
  ...
  if ("kl2" %in% names(attribute_matrix)) {
    attribute_matrix$d_sf6d <- attribute_matrix$d_sf6d - (attribute_matrix$kl2 * utility_params$kl_grades$kl2)
  }
  if ("kl3" %in% names(attribute_matrix)) {
    attribute_matrix$d_sf6d <- attribute_matrix$d_sf6d - (attribute_matrix$kl3 * utility_params$kl_grades$kl3)
  }
  if ("kl4" %in% names(attribute_matrix)) {
    attribute_matrix$d_sf6d <- attribute_matrix$d_sf6d - (attribute_matrix$kl4 * utility_params$kl_grades$kl4)
  }
  
  # --- 2. Decrement from BMI ---
  # This is a linear decrement based on BMI value.
  attribute_matrix$d_sf6d <- attribute_matrix$d_sf6d + (attribute_matrix$d_bmi * utility_params$bmi_decrement)
  
  
  # --- 3. Decrement from TKA Revision ---
  if ("revi" %in% names(attribute_matrix)) {
      attribute_matrix$d_sf6d <- attribute_matrix$d_sf6d - (attribute_matrix$revi * utility_params$revision)
  }

  # --- 4. Decrement from Comorbidities ---
  if (!is.null(utility_params$comorbidities)) {
    for (comorbidity in names(utility_params$comorbidities)) {
      ...
      if (col_name %in% names(attribute_matrix)) {
        ...
        attribute_matrix$d_sf6d <- attribute_matrix$d_sf6d - (attribute_matrix[[col_name]] * decrement)
      }
    }
  }
