...
calculate_revision_risk_fcn <- function(am_curr, rev_coeffs) {
  ...
  # Prevent revision if one has already occurred (rev1 is the state of ever having a revision)
  ...
  am_curr[, c("rev_lpv", "rev_haz_early", "rev_prob_early", "rev_haz_late", "rev_prob_late", "rev_prob") := NULL]
