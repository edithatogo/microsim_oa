...
calculate_revision_risk_fcn <- function(am_curr, rev_coeffs) {
  ...
  am_curr[, rev_lpv :=
    rev_coeffs$linear_predictor$age * .SD$age +
    rev_coeffs$linear_predictor$female * .SD$female +
    rev_coeffs$linear_predictor$bmi * .SD$bmi +
    rev_coeffs$linear_predictor$public * .SD$public, .SDcols = c("age", "female", "bmi", "public")]

  # --- 2. Calculate Hazard for Early Revision (Year 1 post-TKA) ---
  # Individuals in their first year since TKA (agetka1 == 1)
  am_curr[agetka1 == 1,
          rev_haz_early := rev_coeffs$early_hazard$intercept + .SD$rev_lpv, .SDcols = "rev_lpv"]

  # Convert log-hazard to probability
  am_curr[agetka1 == 1,
          rev_prob_early := 1 - exp(-exp(.SD$rev_haz_early)), .SDcols = "rev_haz_early"]
  ...
  am_curr[agetka1 >= 2,
          rev_haz_late := rev_coeffs$late_hazard$intercept +
            rev_coeffs$late_hazard$log_time * log(.SD$agetka1) +
            .SD$rev_lpv, .SDcols = c("agetka1", "rev_lpv")]

  # Convert log-hazard to probability
  am_curr[agetka1 >= 2,
          rev_prob_late := 1 - exp(-exp(.SD$rev_haz_late)), .SDcols = "rev_haz_late"]

  # --- 4. Determine Revision Events ---
  # Combine probabilities and generate random numbers
  am_curr[, rev_prob := 0]
  am_curr[agetka1 == 1, rev_prob := .SD$rev_prob_early, .SDcols = "rev_prob_early"]
  am_curr[agetka1 >= 2, rev_prob := .SD$rev_prob_late, .SDcols = "rev_prob_late"]

  rev_rand <- runif(nrow(am_curr), 0, 1)

  # Determine revision event
  am_curr[, revi := ifelse(.SD$rev_prob > rev_rand, 1, 0), .SDcols = "rev_prob"]

  # --- 5. Clean up and Finalize ---
  # Ensure no revisions for the dead, or those without a TKA
  am_curr[dead == 1 | tka1 == 0, revi := 0]
