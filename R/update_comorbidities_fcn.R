...
update_comorbidities <- function(attribute_matrix, comorbidity_params) {
  ...
  for (comorbidity in names(comorbidity_params$conditions)) {
    ...
    if (length(has_condition_indices) > 0) {
      ...
      attribute_matrix[has_condition_indices, "comorbidity_cost"] <-
        attribute_matrix[has_condition_indices, "comorbidity_cost"] + params$annual_cost

      # Apply the QALY decrement
      if ("d_sf6d" %in% names(attribute_matrix) && "qaly_decrement" %in% names(params)) {
        attribute_matrix[has_condition_indices, "d_sf6d"] <-
          attribute_matrix[has_condition_indices, "d_sf6d"] - params$qaly_decrement
      }
    }
  }
